...
void Init_SRAM(void){
       ...
       SpibRegs.SPIBRR.bit.SPI_BIT_RATE = 5;
...
Uint16 * SRAM_Read(Uint32 Addr, Uint16 size)
{
    EALLOW;
    Uint16 boundary = 0;
    Uint16 * Buffer =  far_malloc(size*sizeof(Uint16));
    //address decoder
    switch(Addr & 0x20000)
    {
...
{
    Uint16 * Buffer =  (Uint16 *)far_malloc(size*sizeof(Uint16));
...
void WriteValue(Uint32 Adrress, Uint16 value, Uint32 size)
{
    ...
    Uint16 * Buffer = copyToBuffer(value, 0x200);
    while (size>0){
        if(size>0x500){
            SRAM_Write(Adrress,Buffer,0x200);
            size = size-0x200;
        }
        else{
